...
import type { Expression } from "calculation-language-language";
import { createCalculationLanguageServices, isExpression } from "calculation-language-language";
...
beforeAll(async () => {
    ...
    const doParse = parseHelper<Expression>(services.CalculationLanguage);
...
function checkDocumentValid(document: LangiumDocument): string | undefined {
    ...
    `
        ...
        || !isExpression(document.parseResult.value) && `Root AST object is a ${document.parseResult.value.$type}, expected a 'Expression'.`
        || undefined;
}
